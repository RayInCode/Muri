    ...
    for job in ${jobs[@]};do
        ...
        for p in ${placement[@]};do
            for s in ${schedule[@]};do
                ...
                # echo ${cmd} 
                python run_sim_744.py --cluster_spec=${cluster_spec} --print --scheme=${p} --trace_file=${job_file} --schedule=${s} --log_path=${log_name} --multi_resource ${multi_resource} >${log_name}/tmp.out &
            done
        done
    done
done
