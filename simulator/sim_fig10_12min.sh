...
for setup in ${setups[@]};do
    ...
    for job in ${jobs[@]};do
        ...
        log_folder="results/result_12min/${setup}j${job}"
        mkdir ${log_folder}
        # echo ${job}
        for p in ${placement[@]};do
            for s in ${schedule[@]};do
                ...
                # echo ${cmd} 
                python run_sim_12min.py --cluster_spec=${cluster_spec} --print --scheme=${p} --trace_file=${job_file} --schedule=${s} --log_path=${log_name} --multi_resource ${multi_resource} >${log_name}/tmp.out &
            done
        done
    done
done
